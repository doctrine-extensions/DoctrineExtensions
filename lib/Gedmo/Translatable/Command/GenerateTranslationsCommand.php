...
EOT
     ...
     */
    private function saveXml($filename, \SimpleXMLElement $xml, $name)
    {
...
EOT;
                if ($annotate) {
                    if ('Entity' ===  $type) {
                        $column = $this->getEntityColumn($mapping, $meta, $tname);
                    } else {
                        $column = $this->getDocumentColumn($mapping);
                    }
                } else {
                    $column = '@var '.$colType;
                }
...
EOT;
    ...
    private function injectTranslationCode($fields, $methods, \ReflectionClass $refl)
    {
        ...
        if (false === file_put_contents($refl->getFileName(), implode("\n", $lines))) {
            throw new \RuntimeException(sprintf('Could not write file %s, check permissions', $refl->getFileName()));
        }
    }

    /**
     * Injects inversed relation code into translated object identified from class reflection
     *
...
EOT;
     ...
     */
    private function getDocumentColumn(array $mapping)
    {
        $field = array();
    ...
    private function getEntityColumn(array $mapping, $meta, $tname)
    {
        ...
        if (isset($mapping['unique']) && $mapping['unique']) {
            throw new \LogicException("Translatable entity: {$meta->name} cannot have unique translatable field"
                .", place unique on {$tname} field '{$mapping['fieldName']}' mapping instead.");
        }
