...
class Yaml extends File
{
    ...
    {
        $mapping = $this->_getMapping($meta->embeddedClasses[$field]['class']);

        // The embedded object must be revisionable
        if (!isset($mapping['gedmo']) && !isset($mapping['gedmo']['revisionable'])) {
            return $config;
        }

        return $this->inspectConfiguration($mapping, $config, $meta, $field);
    }
