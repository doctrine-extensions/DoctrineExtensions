...
class ExtensionMetadataDriverTest extends \PHPUnit_Framework_TestCase
{
     ...
     * @test
     */
    public function shouldInitializeExtensionAnnotationDriver()
    {
        ...
        $emf = new ExtensionMetadataFactory($em, 'Fixture\EncoderExtension', $reader);

        $this->assertInstanceOf('Fixture\EncoderExtension\Mapping\Driver\Annotation', $emf->getExtensionDriver());
    }

    /**
     * @test
     */
    public function shouldFallbackToAnnotationDriverIfTypeIsNotAvailable()
    {
     ...
     */
    public function shouldThrowAnExceptionIfDriverIsNotFound()
    {
        $driver = new YamlDriver(array(__DIR__));

        $em = $this->getEntityManagerMock($driver);
        $reader = $this->getMock('Doctrine\Common\Annotations\AnnotationReader');
        $emf = new ExtensionMetadataFactory($em, 'Fixture\Unknown', $reader);
    }

    /**
     * @test
     */
    public function simplifiedDriversShouldFallback()
    {
