...
class TranslatableTest extends ObjectManagerTestCase
{
     ...
     * @test
     */
    public function shouldTranslateTranslatableEntityFields()
    {
        // test new
        $food = new Post();
        $food->setTitle('Food');
        ...
        $translations = $this->em->getRepository('Gedmo\Fixture\Translatable\PostTranslation')->findAll();
        $this->assertCount(1, $translations, "There should be one english translation available");
        ...
        $food = $this->em->getRepository('Gedmo\Fixture\Translatable\Post')->findOneById($food->getId());
    ...
    public function shouldBeAbleToFallbackToSpecifiedTranslationIfAvailable()
    {
        ...
        $post = $this->em->getRepository('Gedmo\Fixture\Translatable\Post')->findOneById($post->getId());
