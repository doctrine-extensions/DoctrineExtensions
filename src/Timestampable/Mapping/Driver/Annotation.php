        ...
        foreach ($class->getProperties() as $property) {
            ...
            if ($timestampable = $this->reader->getPropertyAnnotation($property, self::TIMESTAMPABLE)) {
                ...
                if ('change' == $timestampable->on) {
                    if (!(property_exists($timestampable, 'field') && $timestampable->field !== null)) {
                        throw new InvalidMappingException("Missing parameters on property - {$field}, field must be set on [change] trigger in class - {$meta->name}");
                    }
                    if (is_array($timestampable->field) && (property_exists($timestampable, 'value') && $timestampable->value !== null)) {
