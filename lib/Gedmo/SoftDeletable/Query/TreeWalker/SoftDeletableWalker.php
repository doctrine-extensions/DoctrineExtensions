...
namespace Gedmo\SoftDeletable\Query\TreeWalker;
...
use Gedmo\SoftDeletable\Query\TreeWalker\Exec\MultiTableDeleteExecutor;
...
class SoftDeletableWalker extends SqlWalker
{
    ...
    public function getExecutor($AST)
    {
        switch (true) {
            ...
            default:
                throw new \Gedmo\Exception\UnexpectedValueException('SoftDeletable walker should be used only on delete statement');
        }
    }
    ...
    private function getSoftDeletableListener()
    {
        if (is_null($this->listener)) {
            ...
            if (is_null($this->listener)) {
                throw new \Gedmo\Exception\RuntimeException('The SoftDeletable listener could not be found.');
            }
        }
    ...
    private function extractComponents(array $queryComponents)
    {
        ...
        foreach ($queryComponents as $alias => $comp) {
            ...
            if ($config && isset($config['softDeletable']) && $config['softDeletable']) {
