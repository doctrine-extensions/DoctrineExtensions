...
use Tree\Fixture\ForeignUuidCategory;
use Tree\Fixture\UuidCategory;
 ...
 */
class NestedTreeUuidTest extends BaseTestCaseORM
{
    const CATEGORY = "Tree\\Fixture\\UuidCategory";
    ...
    public function _testSetParentToNull()
    {
        $repo = $this->em->getRepository(self::CATEGORY);
        $node = $repo->findOneByTitle('Vegitables');
        $node->setParent(null);
    ...
    public function _testTreeUpdateShiftToNextBranch()
    {
        $repo = $this->em->getRepository(self::CATEGORY);
    ...
    public function _testTreeUpdateShiftToUuid()
    {
        $repo = $this->em->getRepository(self::CATEGORY);
        $vegies = $repo->findOneByTitle('Vegitables');
    ...
    public function _testTreeUpdateShiftToOtherParent()
    {
        $repo = $this->em->getRepository(self::CATEGORY);
    ...
    public function _testTreeUpdateShiftToChildParent()
    {
    ...
    public function _testTwoUpdateOperations()
    {
        $repo = $this->em->getRepository(self::CATEGORY);
     ...
     */
    public function _testTreeWithUuidPointingAtAnotherTable()
    {
        // depopulate, i don't want the other stuff in db
        /* * @var NestedTreeRepository $repo */
        $repo = $this->em->getRepository("Tree\\Fixture\\ForeignUuidCategory");
        $all = $repo->findAll();
        foreach ($all as $one) {
            $this->em->remove($one);
        }
    ...
    protected function getUsedEntityFixtures()
    {
        return array(
            self::CATEGORY,
            "Tree\\Fixture\\ForeignUuidCategory",
        );
    }
    ...
    {
        $root = new UuidCategory();
        $root->setTitle("Food");

        $root2 = new UuidCategory();
        $root2->setTitle("Sports");

        $child = new UuidCategory();
        $child->setTitle("Fruits");
        $child->setParent($root);

        $child2 = new UuidCategory();
        $child2->setTitle("Vegitables");
        $child2->setParent($root);

        $childsChild = new UuidCategory();
        $childsChild->setTitle("Carrots");
        $childsChild->setParent($child2);

        $potatoes = new UuidCategory();
