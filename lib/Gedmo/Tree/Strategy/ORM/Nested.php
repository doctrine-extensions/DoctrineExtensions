...
class Nested implements Strategy
{
    ...
    public function processScheduledDelete($em, $node)
    {
        ...
        if ($diff > 2) {
            ...
            if (isset($config['root'])) {
                ...
                $qb->setParameter('rid', $wrappedRootId->getIdentifier(), $type);
            }
            $q = $qb->getQuery();
    ...
    public function updateNode(EntityManagerInterface $em, $node, $parent, $position = 'FirstChild')
    {
        ...
        if (!$isNewNode) {
            ...
            $type = $meta->getTypeOfField($identifierField);
            if (isset($config['root'])) {
                $wrappedNewRoot = AbstractWrapper::wrap($newRoot, $em);
                $newRootId = $wrappedNewRoot->getIdentifier();
                $qb->set('node.' . $config['root'], ':pid');
                $qb->setParameter('pid', $newRootId, $type);
            ...
            if (isset($newParent)) {
                ...
                $qb->setParameter('pid', $newParentId, $type);
                $wrapped->setPropertyValue($config['parent'], $newParent);
            ...
            $qb->setParameter('id', $nodeId, $type);
    ...
    public function shiftRangeRL(EntityManagerInterface $em, $class, $first, $last, $delta, $root = null, $destRoot = null, $levelDelta = null)
    {
        ...
        if (isset($config['root'])) {
            ...
            $qb->setParameter('drid', $destRoot, $type);
            $qb->andWhere($qb->expr()->eq('node.' . $config['root'], ':rid'));
