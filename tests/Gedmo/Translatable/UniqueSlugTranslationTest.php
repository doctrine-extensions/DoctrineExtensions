...
use Gedmo\Fixture\Translatable\Sluggable\Post;
use Gedmo\Fixture\Translatable\Sluggable\PostTranslation;
...
class UniqueSlugTranslationTest extends ObjectManagerTestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->createSchema($this->em, array(
            'Gedmo\Fixture\Translatable\Sluggable\Post',
            'Gedmo\Fixture\Translatable\Sluggable\PostTranslation',
        ));
    }
     ...
     * @test
     */
    public function shouldTranslateTranslatableEntityFieldsByPersistingTranslation()
    {
        // test new
        $food = new Post();
        ...
        $post = $this->em->getRepository('Gedmo\Fixture\Translatable\Sluggable\Post')->findOneById($food->getId());
        $this->assertSame('food-3', $post->getSlug());

        $post = $this->em->getRepository('Gedmo\Fixture\Translatable\Sluggable\Post')->findOneById($next->getId());
     ...
     * @test
     */
    public function shouldTranslateTranslatableEntityFieldsNaturally()
    {
        // test new
        $food = new Post();
        $food->setTitle('Food'); // en translation through default
        ...
        $food = $this->em->getRepository('Gedmo\Fixture\Translatable\Sluggable\Post')->findOneById($food->getId());
        $this->assertSame('food', $food->getSlug());

        $next = $this->em->getRepository('Gedmo\Fixture\Translatable\Sluggable\Post')->findOneById($next->getId());
        $this->assertSame('carrot', $next->getSlug());
    }
