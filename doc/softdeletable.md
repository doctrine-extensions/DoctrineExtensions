# SoftDeletable behavior extension for Doctrine 2

**SoftDeletable** behavior allows to "soft delete" objects, filtering them
...
With SoftDeletable there's one more step you need to do. You need to add the filter to your configuration:
...
$config->addFilter('soft-deletable', 'Gedmo\SoftDeletable\Filter\SoftDeletableFilter');
```

And then you can access the filter from your EntityManager to enable or disable it with the following code:

``` php
// This will enable the SoftDeletable filter, so entities which were "soft-deleted" will not appear
// in results
// You should adapt the filter name to your configuration (ex: softdeletable)
$em->getFilters()->enable('soft-deletable');

// This will disable the SoftDeletable filter, so entities which were "soft-deleted" will appear in results
$em->getFilters()->disable('soft-deletable');
```
...
// in results
// You should adapt the filter name to your configuration (ex: softdeletable)
$em->getFilterCollection()->enable('soft-deletable');

// This will disable the SoftDeletable filter, so entities which were "soft-deleted" will appear in results
$em->getFilterCollection()->disable('soft-deletable');
```

**NOTE:** by default all filters are disabled, so you must explicitly enable **soft-deletable** filter in your setup
or whenever you need it.

<a name="entity-mapping"></a>

## SoftDeletable Entity example:

### SoftDeletable annotations:
- **@Gedmo\Mapping\Annotation\SoftDeletable** this class annotation tells if a class is SoftDeletable. It has a
...
**Note:** that SoftDeletable interface is not necessary, except in cases where
you need to identify entity as being SoftDeletable. The metadata is loaded only once then
cache is activated.
...
<doctrine-mapping xmlns="http://doctrine-project.org/schemas/orm/doctrine-mapping"
    ...
    <entity name="Mapping\Fixture\Xml\Timestampable" table="timestampables">
        ...
        <gedmo:soft-deletable field-name="deletedAt" time-aware="false" />
    </entity>
...
$filter = $em->getFilters()->enable('soft-deletable');
