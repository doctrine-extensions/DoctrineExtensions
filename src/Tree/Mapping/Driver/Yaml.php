    ...
    public function readExtendedMetadata($meta, array &$config)
    {
        ...
        if (isset($mapping['id'])) {
            foreach (($mapping->id ?? $mapping['id']) as $field => $fieldMapping) {
                if (isset($fieldMapping['gedmo'])) {
                    if (in_array('treePathSource', $fieldMapping->gedmo ?? $fieldMapping['gedmo'], true)) {
        ...
        if (isset($mapping['fields'])) {
            foreach (($mapping->fields ?? $mapping['fields']) as $field => $fieldMapping) {
                if (isset($fieldMapping['gedmo'])) {
                    if (in_array('treeLeft', $fieldMapping->gedmo ?? $fieldMapping['gedmo'], true)) {
                        ...
                        $config['root'] = $field;
                    } elseif (in_array('treePath', $fieldMapping->gedmo ?? $fieldMapping['gedmo'], true) || isset($fieldMapping['gedmo']['treePath'])) {
                        ...
                        $treePathInfo = ($fieldMapping->gedmo ?? $fieldMapping['gedmo'])['treePath'] ?? ($fieldMapping->gedmo ?? $fieldMapping['gedmo'])[array_search(
                            'treePath',
                            $fieldMapping->gedmo ?? $fieldMapping['gedmo'],
                            true
                        )];
                    ...
                    } elseif (in_array('treePathSource', $fieldMapping->gedmo ?? $fieldMapping['gedmo'], true)) {
                        if (!$validator->isValidFieldForPathSource($meta, $field)) {
                            throw new InvalidMappingException("Tree PathSource field - [{$field}] type is not valid. It can be any of the integer variants, double, float or string in class - {$meta->getName()}");
                        }
                        $config['path_source'] = $field;
                    } elseif (in_array('treePathHash', $fieldMapping->gedmo ?? $fieldMapping['gedmo'], true)) {
                        if (!$validator->isValidFieldForPathSource($meta, $field)) {
                    ...
                    } elseif (in_array('treeLockTime', $fieldMapping->gedmo ?? $fieldMapping['gedmo'], true)) {
                    ...
                    } elseif (in_array('treeParent', $fieldMapping->gedmo ?? $fieldMapping['gedmo'], true)) {
                        $config['parent'] = $field;
                    }
                }
            }
